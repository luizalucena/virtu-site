...
(function () {
  ...
  css.textContent = [
    '#vt-wa-btn { position:fixed; bottom:24px; right:24px; width:56px; height:56px; background:#1a2a4a; border:1px solid rgba(184, 148, 63,.45); border-radius:50%; display:flex; align-items:center; justify-content:center; box-shadow:0 6px 20px rgba(26,38,64,0.28); z-index:500; text-decoration:none; transition:transform .2s,box-shadow .2s,bottom .25s ease; }',
    '#vt-wa-btn:hover { transform:scale(1.08); box-shadow:0 10px 28px rgba(26,38,64,0.36); }',
    '#vt-wa-btn svg { width:28px; height:28px; fill:#b8943f; }',
    '#vt-wa-tip { position:fixed; bottom:90px; right:24px; background:#14223c; color:#FAF8F5; font-size:12px; font-family:Cormorant Garamond,Georgia,serif; letter-spacing:.06em; padding:7px 13px; border-radius:10px; white-space:nowrap; opacity:0; pointer-events:none; transition:opacity .2s; z-index:499; }',
